...
function draw() {
    ...
    // display oranges
    for (let i = 0; i < oranges.length; i++) {
        image(orangeImg, oranges[i].x, oranges[i].y, 70, 70);
    }
    ...
    if (gameState === "playing") {
        ...
        // orange collision
        for (let i = 0; i < oranges.length; i++) {
            let d = dist(birdX, birdY, oranges[i].x, oranges[i].y);
    ...
    textSize(14);
    text("Move the pigeon with WASD", 10, 75);
